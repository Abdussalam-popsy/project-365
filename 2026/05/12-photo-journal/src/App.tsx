...
/** Strip cell width — must match `width` on each strip `motion.div`. */
const PHOTO_WIDTH = 260;
/** Height of each photo cell in the strip (and the polaroid front photo). */
const STRIP_HEIGHT = 270;
/** Distance from viewport top to the strip container's top edge. */
const STRIP_TOP = 393;
/** Vertical center of the strip band — shared anchor for strip AND polaroid. */
const STRIP_CENTER_Y = STRIP_TOP + STRIP_HEIGHT / 2; // 528px
/** Center-to-center spacing when spread (Figma: 260px card + 25px gap → 285px). */
const PHOTO_CENTER_STEP = 285;

// X offset of each photo's center from the viewport center when fully spread (left: 50% strip).
const SPREAD_X = IMAGES.map((_, i) =>
  Math.round((i - (IMAGES.length - 1) / 2) * PHOTO_CENTER_STEP),
);
/** Centers a `PHOTO_WIDTH` photo on the strip's `left: 50%` anchor (translate origin). */
const CENTER_X = -PHOTO_WIDTH / 2;
...
function PolaroidCard({
  ...
  return (
    <motion.div
      ...
      style={{ top: STRIP_CENTER_Y, perspective: 1200 }}
        ...
        >
          ...
          <div
            style={{
              ...
              gap: 8,
              boxShadow:
                "0px 100px 43px rgba(0,0,0,0.07), 0px 41.778px 17.964px rgba(0,0,0,0.05), 0px 22.336px 9.605px rgba(0,0,0,0.04), 0px 12.522px 5.384px rgba(0,0,0,0.03)",
            }}
          >
            <div
              style={{
          ...
          >
            ...
            <p style={{ margin: 0 }}>
              I have come to you bearing sad news of my recent escapade. A DATE.
              as you might have guessed, it went horribly wrong.
            </p>
            <div
              style={{
                flex: 1,
