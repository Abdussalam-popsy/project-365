...
const SCENE_IDS = ["idle", "house"] as const;
...
async function loadSceneBitmap(url: string): Promise<Set<number>> {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      const H2 = TILE_PX >> 1, H4 = TILE_PX >> 2, H34 = H4 * 3;
      ...
      for (let row = 0; row < SCENE_ROWS; row++) {
        for (let col = 0; col < SCENE_COLS; col++) {
          ...
          for (const [px, py] of pts) {
            const sx = Math.min(px | 0, W - 1), sy = Math.min(py | 0, H - 1);
...
export function TileGrid() {
  ...
  useEffect(() => {
    ...
    const handler = (e: MediaQueryListEvent) => setSceneScale(e.matches ? 2 : 1);
    ...
    function onScroll() {
      ...
      raf = requestAnimationFrame(() => {
        ...
        colRefs.current.forEach((el, col) => {
          if (el) el.style.transform = `translateY(${Math.sin(col * 0.7) * sy * 0.03}px)`;
        });
      });
    }
  ...
  return (
    <div
      ...
      onClick={handleClick}
    >
      <div style={{ display: "flex", gap: `${GAP_PX}px`, alignItems: "flex-start" }}>
                ...
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: sceneActive
                      ? (lit ? tiles[col + row * cols] : SCENE_BG)
                      : tiles[col + row * cols],
                    borderRadius:    0,
